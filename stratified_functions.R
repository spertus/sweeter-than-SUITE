...
run_stratified_simulation <- function(population, strata, sample_sizes, mu_0 = 0.5, n_sims = 300, alpha = .05, bounds = c(0,1), pars){
  ...
  power_alpha_aggressive_unstrat <- matrix(NA, nrow = nrow(sample_sizes), ncol = 1)
  power_alpha_aggressive_fisher <- matrix(NA, nrow = nrow(sample_sizes), ncol = 1)
  power_alpha_aggressive_martingale <- matrix(NA, nrow = nrow(sample_sizes), ncol = 1)
  
  #need to average the eta_0 for the unstratified version of ALPHA
  unstrat_pars <- pars
  unstrat_pars$eta_0 <- mean(pars$eta_0)
  pars_aggressive <- list(d = 2000, eta_0 = c(1,1))
  unstrat_pars_aggressive <- list(d = 2000, eta_0 = 1)
  ...
  for(i in 1:nrow(sample_sizes)){
    ...
    replicates_alpha_aggressive_unstrat <- replicate(n = n_sims, get_stratified_pvalue(population = population, strata = rep(1, length(population)), mu_0 = mu_0, n = sum(sample_sizes[i,]), method = "alpha", bounds = bounds, pars = unstrat_pars_aggressive))
    replicates_alpha_aggressive_fisher <- replicate(n = n_sims, get_stratified_pvalue(population = population, strata = strata, mu_0 = mu_0, n = sample_sizes[i,], method = "alpha", pool = "fisher", bounds = bounds, pars = pars_aggressive))
    replicates_alpha_aggressive_martingale <- replicate(n = n_sims, get_stratified_pvalue(population = population, strata = strata, mu_0 = mu_0, n = sample_sizes[i,], method = "alpha", pool = "martingale", bounds = bounds, pars = pars_aggressive))
    ...
    power_alpha_aggressive_unstrat[i,1] <- mean(replicates_alpha_unstrat < alpha)
    power_alpha_aggressive_fisher[i,1] <- mean(replicates_alpha_fisher < alpha)
    power_alpha_aggressive_martingale[i,1] <- mean(replicates_alpha_martingale < alpha)
  ...
  colnames(power_alpha_aggressive_fisher) <- "power_alpha_aggressive_fisher"
  colnames(power_alpha_aggressive_martingale) <- "power_alpha_aggressive_martingale"
  colnames(power_alpha_aggressive_unstrat) <- "power_alpha_aggressive_unstrat"
  ...
  power_frame <- data.frame(
    ...
    power_alpha_unstrat,
    power_alpha_aggressive_fisher,
    power_alpha_aggressive_martingale,
    power_alpha_aggressive_unstrat,
    power_eb_fisher,
    power_eb_martingale,
    power_eb_unstrat, 
    power_hedged_fisher,
...
get_two_strata_alpha <- function(stratum_1, stratum_2, replace, A_c, u, d = NULL, eta_0 = NULL, f = NULL, min_SD = .01, rule = "equal", resolution = NULL, mu_0 = 0.5, combine = "product"){
  ...
  if(!is.null(eta_0) & !is.null(d)){
    if(is.null(f)){f <- c(0,0)}
    sigma_1 <- pmax(c(1/2, lag(get_sigma_hat_squared(shuffled_1))[2:N[1]]), min_SD)
    sigma_2 <- pmax(c(1/2, lag(get_sigma_hat_squared(shuffled_2))[2:N[2]]), min_SD)
    eta_1 <- matrix(((d[1]*eta_0[1] + c(0, lag(cumsum(shuffled_1), 1)[2:N[1]])) / (d[1] + 1:N[1] - 1) + f[1] * u[1]/sigma_1) / (1 + f[1] / sigma_1), nrow = N[1], ncol = ncol(m_1))
    eta_2 <- matrix(((d[2]*eta_0[2] + c(0, lag(cumsum(shuffled_2), 1)[2:N[2]])) / (d[2] + 1:N[2] - 1) + f[2] * u[2]/sigma_2) / (1 + f[2] / sigma_2), nrow = N[2], ncol = ncol(m_2))
  } else{
